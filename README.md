# meccha camouflage V2.1


# What's Changed?

- new setup system fixing any issues
- fixed menu issues (not launch)
- better colloring in auto paint
- faster painting
( new! i only updated the launch and so it works for new update )

# SetUp / download:
(Follow the guide)

 1. - 1 You Will Need The Following:
winrar,
python,

 2. - 2 before download you will need to turn off smart app control and windows Defender..
(if you are not sure about this it is beacuse windows defender takes closed source as a "virus" and smart app control wont let you open closed source projects if they do not have a verified realeser)

 3. - 3 download "meccha camouflage V2.1" then create a folder on your desktop and call it whatever you want now put the files from the winrar into your new folder..
(password is "Meccha")

 4. - 4 open "main (open to start).bat" as admin if not it wont open (only way i could fix this)..

and done

# Fixes:
(common issues with fixes)

- "wont open at all" try to install NET run10 < - this should be fixed but if not then do this
- "load.error" you didnt turn off your antivirus and have to reinstall the whole project since it has deleted files

# Thanks For Using meccha camouflage

buy me a coffie
btc: 1Q2TWHE3GMdB6BZKafqwxXtWAWgFt5Jvm3

just updated some files and so it works for new update
